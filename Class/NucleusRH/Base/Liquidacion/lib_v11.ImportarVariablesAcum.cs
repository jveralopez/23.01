...
using NucleusRH.Base.Liquidacion.InterfaceVariablesAcum;
using NucleusRH.Base.Liquidacion.Legajo_Liquidacion;

namespace NucleusRH.Base.Liquidacion_VarAcum
{

    class clsImportarVariables
    {

        /// <summary>
        /// Constructor
        /// </summary>
        public clsImportarVariables()
        {
        }

        /// <summary>
        /// Importa los acumuladores de personal
        /// </summary>
        /// <param name="pstrOiLiquidacion">OI de la liquidación</param>
        public void ImportarVariablesPersona(string pstoi_empresa)
        {
            ...
            PERSONAL_EMP objPersonalEmp;
            VAL_VAREA objVarPersona;
            stuVariable objVariable;

            //Obtiene las variables y las guarda en un hashtable
            htaVariables = this.GetVariables("5"); //Solo variables de tipo FIJO
            ...
            while (xtrOIA.Read())
            {
                ...
                NomadProxy.GetProxy().Batch().Trace.Add("ifo", "Procesando la variable '" + objVarArchivo.c_variable + "' para el legajo '" + objVarArchivo.e_numero_legajo.ToString() + "'.", "ImportarVariablesPersona");

                //Valida que la variable exista entre las posibles a importar
                if (!htaVariables.ContainsKey(objVarArchivo.c_variable))
                {
                    NomadProxy.GetProxy().Batch().Trace.Add("err", "La variable '" + objVarArchivo.c_variable + "' no existe o no es de tipo Acumulador de Legajo.", "ImportarVariablesPersona");
                    NomadEnvironment.GetTrace().Error("La variable '" + objVarArchivo.c_variable + "' no existe o no es de tipo Acumulador de Legajo.");
                    continue;
                }

                objVariable = (stuVariable)htaVariables[objVarArchivo.c_variable];
                ...
                {
                    NomadProxy.GetProxy().Batch().Trace.Add("err", "Se produjo un error al tratar de obtener el valor de la variable '" + objVarArchivo.c_variable + "'. " + ex.Message, "ImportarVariablesPersona");
                    NomadEnvironment.GetTrace().Error("Se produjo un error al tratar de obtener el valor de la variable '" + objVarArchivo.c_variable + "'. " + ex.Message);
                    continue;
                }
                ...
                if (htaPersonalLiqs.ContainsKey(objVarArchivo.e_numero_legajo.ToString()))
                {
                    ...
                    objPersonalEmp = (PERSONAL_EMP)htaPersonalLiqs[objVarArchivo.e_numero_legajo.ToString()];
                }
                else
                {
                    //Obtiene el PersonalLiq desde la DB y lo guarda en la hash
                    strOiPersonalLiq = this.GetOIPersonal(pstoi_empresa, objVarArchivo.e_numero_legajo.ToString());

                    //Valida que el legajo exista en la liquidación
                    if (strOiPersonalLiq == "")
                    {
                        NomadProxy.GetProxy().Batch().Trace.Add("err", "El legajo '" + objVarArchivo.e_numero_legajo + "' no está asociado a la  empresa.", "ImportarVariablesPersona");
                        NomadEnvironment.GetTrace().Error("El legajo '" + objVarArchivo.e_numero_legajo + "' no está asociado a la empresa.");
                        continue;
                    }

                    objPersonalEmp = PERSONAL_EMP.Get(strOiPersonalLiq);
                    htaPersonalLiqs.Add(objVarArchivo.e_numero_legajo.ToString(), objPersonalEmp);
                }

                //Busca la variable dentro de la colección de variables
                objVarPersona = (VAL_VAREA)objPersonalEmp.VAL_VAREA.GetByAttribute("oi_variable", objVariable.OI);
                if (objVarPersona == null)
                {
                    //No existe en la colección, la crea y la agrega
                    objVarPersona = new VAL_VAREA();
                    objVarPersona.oi_variable = objVariable.OI;
                    objVarPersona.n_valor = dblValor;

                    objPersonalEmp.VAL_VAREA.Add(objVarPersona);
                }
                else
                {
        ...
        private Hashtable GetVariables(string pstrTipo)
        {
            ...
            string strResult;
            string strParametros = "<DATOS tipo_var=\"" + pstrTipo + "\" />";
            stuVariable objVariable;
            ...
            while (xtrLP.Read())
            {
                ...
                htaResult.Add(xtrLP.GetAttribute("c_variable"), objVariable);
            }
        ...
        private string GetOIPersonal(string pstrOiEmpresa, string pstrLegajo)
        {
            string strRows;
            string strResult = "";
            string strParametros = "<DATOS oi_empresa=\"" + pstrOiEmpresa + "\" e_numero_legajo=\"" + pstrLegajo + "\" />";

            strRows = NomadProxy.GetProxy().SQLService().Get(ENTRADA.Resources.qry_persona_oi, strParametros);

            XmlDocument objXML = new XmlDocument();
            objXML.LoadXml(strRows);

            if (objXML.DocumentElement.HasAttribute("oi_personal_emp"))
                strResult = objXML.DocumentElement.GetAttribute("oi_personal_emp");

...
using NucleusRH.Base.Liquidacion.InterfaceVariablesFijas;
using NucleusRH.Base.Liquidacion.Legajo_Liquidacion;

namespace NucleusRH.Base.Liquidacion_VarFija
{
    ...
    class clsImportarVariables
    {
        ...
        public void ImportarVariablesPersona(string pstoi_empresa)
        {
            ...
            VAL_VAREF objVarPersona;
            stuVariable objVariable;

            //Obtiene las variables y las guarda en un hashtable
            htaVariables = this.GetVariables("3"); //Solo variables de tipo FIJO
            ...
            while (xtrOIA.Read())
            {
                ...
                {
                    NomadProxy.GetProxy().Batch().Trace.Add("err", "La variable '" + objVarArchivo.c_variable + "' no existe o no es de tipo Novedad Fija de Legajo.", "ImportarVariablesPersona");
                    NomadEnvironment.GetTrace().Error("La variable '" + objVarArchivo.c_variable + "' no existe o no es de tipo Novedad fija de Legajo.");
                    continue;
                }
                ...
                objVarPersona = (VAL_VAREF)objPersonalEmp.VAL_VAREF.GetByAttribute("oi_variable", objVariable.OI);
                if (objVarPersona == null)
                {
                    //No existe en la colección, la crea y la agrega
                    objVarPersona = new VAL_VAREF();
                    objVarPersona.oi_variable = objVariable.OI;
                    objVarPersona.n_valor = dblValor;

                    objPersonalEmp.VAL_VAREF.Add(objVarPersona);
                }
                else
                {

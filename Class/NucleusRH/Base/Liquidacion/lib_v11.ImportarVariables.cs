...
using NucleusRH.Base.Liquidacion.InterfaceVariables;
...
namespace NucleusRH.Base.Liquidacion {
    ...
    class clsImportarVariables {
        ...
        public void ImportarVariablesPersona(string pstrOiLiquidacion) {
            ...
            htaVariables = this.GetVariables("1"); //Solo variables de tipo NovedadPersona
        ...
        if (!xtrOIA.IsStartElement())
          continue;
                ...
                if (!htaVariables.ContainsKey(objVarArchivo.c_variable)) {
                    NomadProxy.GetProxy().Batch().Trace.Add("err", "La variable '" + objVarArchivo.c_variable + "' no existe o no es de tipo Novedad de Legajo.", "ImportarVariablesPersona");
                    NomadEnvironment.GetTrace().Error("La variable '" + objVarArchivo.c_variable + "' no existe o no es de tipo Novedad de Legajo.");
                    continue;
                }
                ...
                if (htaPersonalLiqs.ContainsKey(objVarArchivo.e_numero_legajo.ToString())) {
                    ...
                    NomadEnvironment.GetTrace().Info("OBJ: " + objVarArchivo.e_numero_legajo.ToString());
                    NomadEnvironment.GetTrace().Info("OI: " + pstrOiLiquidacion);

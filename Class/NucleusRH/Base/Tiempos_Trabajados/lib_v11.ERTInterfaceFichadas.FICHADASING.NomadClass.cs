...
namespace NucleusRH.Base.Tiempos_Trabajados.ERTInterfaceFichadas 
{
  ...
  //Clase FichadasIngresadas
    public partial class FICHADASING : Nomad.NSystem.Base.NomadObject
    {
        ...
        {
            //Codigo en .NET
            NomadBatch b = NomadBatch.GetBatch("Importar Fichadas ERT", "Importar Fichadas ERT");
            ...
            NucleusRH.Base.Tiempos_Trabajados.ERTInterfaceFichadas.FICHADASING intFichIng;
            ...
            for (linea = 1, MyROW = MyXML.FirstChild().FirstChild(); MyROW != null; linea++, MyROW = MyROW.Next())
            {
                ...
                try
                {
                    ...
                    //TIPO
                    //ILT-2017.06.12
                    if (IOValidator.IsInput(intFichIng.sentido.ToUpper()))
                    {
                        ddoFichIng.c_fichadasing = Nomad.NSystem.Functions.StringUtil.date2str(ddoFichIng.f_fechahora) + ddoFichIng.e_numero_legajo.ToString() + "AE";
                        ddoFichIng.l_entrada = true;
                        ddoFichIng.c_tipo = "E";	
                    }
                    else if (IOValidator.IsOutput(intFichIng.sentido.ToUpper()))
                    {
                        ddoFichIng.c_fichadasing = Nomad.NSystem.Functions.StringUtil.date2str(ddoFichIng.f_fechahora) + ddoFichIng.e_numero_legajo.ToString() + "AS";
                        ddoFichIng.l_entrada = false;
                        ddoFichIng.c_tipo = "S";	
                    }
                    
                    /* ILT-2017.06.12
                    switch (intFichIng.sentido)
                    {
                        case "0":
                            ddoFichIng.c_fichadasing = Nomad.NSystem.Functions.StringUtil.date2str(ddoFichIng.f_fechahora) + ddoFichIng.e_numero_legajo.ToString() + "AE";
                            ddoFichIng.l_entrada = true;

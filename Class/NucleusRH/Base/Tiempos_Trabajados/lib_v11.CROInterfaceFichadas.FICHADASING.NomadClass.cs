...
namespace NucleusRH.Base.Tiempos_Trabajados.CROInterfaceFichadas 
{
  ...
  //Clase FichadasIngresadas
    public partial class FICHADASING : Nomad.NSystem.Base.NomadObject
    {
        ...
        {
            NomadBatch b = NomadBatch.GetBatch("Importar Fichadas CRO", "Importar Fichadas CRO");
            ...
            NucleusRH.Base.Tiempos_Trabajados.CROInterfaceFichadas.FICHADASING intFichIng;
            ...
            */
            ...
            for (linea = 1, MyROW = MyXML.FirstChild().FirstChild(); MyROW != null; linea++, MyROW = MyROW.Next())
            {
                ...
                try
                {
                    ...
                    if (IOValidator.IsInput(intFichIng.lector.ToUpper() + "-" + intFichIng.evento.ToUpper())
	                    || IOValidator.IsInput("*-" + intFichIng.evento.ToUpper())
	                    || IOValidator.IsInput(intFichIng.lector.ToUpper() + "-*")
	                    || IOValidator.IsInput("*-*")
	                    || IOValidator.IsInput(intFichIng.evento.ToUpper()))
	                {
                    ...
                    else if (IOValidator.IsOutput(intFichIng.lector.ToUpper() + "-" + intFichIng.evento.ToUpper())
		                    || IOValidator.IsOutput("*-" + intFichIng.evento.ToUpper())
		                    || IOValidator.IsOutput(intFichIng.lector.ToUpper() + "-*")
                    ...
                    if ((Ent.Count != 0) || (Sal.Count != 0))
                    {
                        ...
                        if (!found)
                        {
                            auxdat = intFichIng.lector.ToUpper() + "-" + intFichIng.evento.ToUpper();
                        ...
                        if (!found)
                        {
                            auxdat = intFichIng.lector.ToUpper() + "-*";
                            if (Ent.ContainsKey(auxdat))
                            {
                                ddoFichIng.l_entrada = true;
                                ddoFichIng.c_fichadasing += "AE";
                        ...
                        {
                            ficrec++;
                            b.Wrn("El evento '" + intFichIng.lector.ToUpper() + "-" + intFichIng.evento.ToUpper() + "' no reconocido. Linea " + linea.ToString());
                            continue;
                        }
                    }
                    else

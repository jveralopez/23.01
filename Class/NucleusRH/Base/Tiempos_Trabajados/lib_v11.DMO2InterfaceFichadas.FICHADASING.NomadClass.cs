...
namespace NucleusRH.Base.Tiempos_Trabajados.DMO2InterfaceFichadas
{
  ...
  //Clase FichadasIngresadas
    public partial class FICHADASING : Nomad.NSystem.Base.NomadObject
    {
        ...
        {
            //Codigo en .NET
            NomadBatch b = NomadBatch.GetBatch("Importar Fichadas DMO2", "Importar Fichadas DMO2");
            ...
            NucleusRH.Base.Tiempos_Trabajados.DMO2InterfaceFichadas.FICHADASING intFichIng;
            ...
            for (linea = 1, MyROW = MyXML.FirstChild().FirstChild(); MyROW != null; linea++, MyROW = MyROW.Next())
            {
                ...
                {   
					...
					if(intFichIng.fecha.ToUpper() == "DATE")
					{
						b.Log("Se Saltea el registro de cabecera");
						continue;
					}
					...
					//Fichada de entrada
                    if (intFichIng.hora_entrada != "-")
                    {	
						//Genero un DateTime para HE con cualquier fecha, Solo se utiliza la hora
						System.DateTime HE = StringUtil.str2date(intFichIng.hora_entrada.Replace(":",""));
					...
					//Fichada de Salida
                    if (intFichIng.hora_salida != "-")
                    {	
						//Genero un DateTime para HS con cualquier fecha, Solo se utiliza la hora
						System.DateTime HS = StringUtil.str2date(intFichIng.hora_salida.Replace(":",""));

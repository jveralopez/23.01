...
namespace NucleusRH.Base.Tiempos_Trabajados.SISTEMICAInterfaceFichadas
{
    ...
    public partial class FICHADASING : Nomad.NSystem.Base.NomadObject
    {
        ...
        {
            //Codigo en .NET
            NomadBatch b = NomadBatch.GetBatch("Importar Fichadas SISTEMICA", "Importar Fichadas SISTEMICA");
            ...
            NucleusRH.Base.Tiempos_Trabajados.SISTEMICAInterfaceFichadas.FICHADASING intFichIng;
            ...
            for (linea = 1, MyROW = MyXML.FirstChild().FirstChild(); MyROW != null; linea++, MyROW = MyROW.Next())
            {
                ...
                try
                {
                    ...
                    oi_personal = NucleusRH.Base.Tiempos_Trabajados.FichadasIng.FICHADASING.GetPersonalEmpresaIDFromCard(intFichIng.tarjeta);
                    if (oi_personal == 0)
                    {
                        ficrec++;
                        b.Wrn("Tarjeta " + intFichIng.tarjeta + " no encontrada... Linea " + linea.ToString());
                        continue;
                    }
                    ...
                    System.DateTime FH = new System.DateTime(int.Parse(intFichIng.fecha.Substring(6, 4)), int.Parse(intFichIng.fecha.Substring(3, 2)), int.Parse(intFichIng.fecha.Substring(0, 2)), int.Parse(intFichIng.hora.Substring(0, 2)), int.Parse(intFichIng.hora.Substring(3, 2)), 0);

...
alter table dbo.ORG19_ZONAS
   add constraint AK_AK1_ORG_ZONAS_ORG19_ZO unique (c_zona, oi_localidad)
go
...
alter table dbo.ORG31_REGIONES
   add constraint AK_REGIONES_ORG31_RE unique (c_region)
go

alter table dbo.ORG19_PROVINCIAS
	add oi_region int null
go

alter table dbo.ORG19_PROVINCIAS
   add constraint FK_ORG19_PR_REFERENCE_ORG31_RE foreign key (oi_region)
...
alter table dbo.ORG32_MUNICIPIOS
   add constraint AK_MUNICIPIOS_ORG32_MU unique (c_municipio)
go

alter table dbo.ORG19_LOCALIDADES
	add oi_municipio int null
go

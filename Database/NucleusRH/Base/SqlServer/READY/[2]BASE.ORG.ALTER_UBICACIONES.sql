...
select @ScripName = 'BASE.ORG.ALTER_UBICACIONES', @n_Version = 1 , @d_modulo ='ORG'
...
/*==============================================================*/
IF NOT EXISTS (SELECT  * FROM  sysobjects NOLOCK  WHERE name ='ORG03_UBICACIONES' and id in 
		(SELECT  id FROM  syscolumns NOLOCK  WHERE name in ('n_pais','n_area','n_telefono','c_afip','c_estado','oi_juris_afip' )) )
BEGIN
	...
	EXEC dbo.sp_NMD_LogScript @oi_Version  ,@ScripName , @d_modulo, ' ADD n_pais,n_area,n_telefono,c_afip,c_estado,oi_juris_afip  ' , @@error , ''
END
...
IF NOT EXISTS (SELECT  * FROM  sysobjects NOLOCK  WHERE name ='ORG03_UBICACIONES' and id in 
		(SELECT  parent_obj FROM  sysobjects  NOLOCK  WHERE name ='FK_ORG03_UB_REFERENCE_ORG36_JU' ) )
BEGIN
	...
	EXEC dbo.sp_NMD_LogScript @oi_Version  ,@ScripName , @d_modulo, 'add constraint  FK_ORG03_UB_REFERENCE_ORG36_JU' , @@error , ''
END
